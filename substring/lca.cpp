...
const int largest_num=pow(2.0,32.0)-1;
lca_info *lca_values;
int l,Iw,k;
...
void preprocess_suffixtree(suffixtree_node *stree,bfslist_suffixtree & bfslist)
{
	...
	lca_values=(lca_info*)malloc(suffixtree_node_count*sizeof(lca_info));
	...
	while(!s.empty())
	{
		...
		lca_values[crnt_node->num].h= ceil( log(crnt_node->num&-crnt_node->num) / log(2.0) ) +1;
		s.pop();
...
suffixtree_node* closest(suffixtree_node *a,int A,int j)
{
	...
	else
	{
		...
		Iw= (largest_num & (1 << (k-1))) & lca_values[a->num].I;
...
suffixtree_node* calculate_lca(suffixtree_node *x,suffixtree_node *y)
{
    ...
    leftmost1 = 32 - ceil( log(leftmost1) / log(2.0) );
	
	b= (largest_num & (1 << (leftmost1-1))) & lca_values[x->num].I;//x->num & (1 << (leftmost1-1)); //lca value
